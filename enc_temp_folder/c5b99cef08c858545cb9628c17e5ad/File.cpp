...
TrainingSet *File::readTrainingSet() {
    ...
    for(int i = 0;; i++) {
        ...
        ret->outputMap[output] = sanitize(buffer, false);
		getline(m_file, buffer);
        check();
        ...
        while(1) {
            ...
            if(maxLength < len)
                maxLength = len;
			curr.input = stringToInput(sanitize(buffer));
...
ANN *File::readANN() {
	cout << "Reading the neural network in " << m_path << endl;
	open(); //Openning the file in read-only mode
	unsigned int progress = 0;
    string buffer;
...
WordClassifier *File::readWordClassifier() {
	cout << "Reading the word classifier in " << m_path << ':' << endl;
	open(); //Openning the file in read-only mode
	string buffer;
...
int File::countLines() {
    int ret = 0;
    string garbage;
	streampos before = m_file.tellg();
	m_file.seekg(0, fstream::beg);
    while(getline(m_file, garbage))
        ret++;
    m_file.clear();
	m_file.seekg(before, fstream::beg);
    return ret;
}